...
for pol in politicians:
	if pol['latest_member']['house'] == 'senate':
		currentSenators.append(pol['id'])
...
for pol in politicians:
	if pol['latest_member']['house'] == 'senate':
		...
		#store results in cumulativeAgreement

		cumulativeAgreement = {}

		polName = pol['latest_member']['name']['first'] + ' ' + pol['latest_member']['name']['last']

		cumulativeAgreement['keyName'] = polName
		cumulativeAgreement['cleanKeyName'] = polName.lower().replace(" ","_")	

		polId = pol['id']

		cumulativeAgreement['memberId'] = polId

		print "Checking", polName, "with id", polId

		#Then check every division

		#Set up  a dict to store cumulative agreement counts for other senators

		voteChecks = {}

		for otherPol in politicians:
			if otherPol['latest_member']['house'] == 'senate' and otherPol['id'] != polId:
				data = {}
				otherCleanName = otherPol['latest_member']['name']['first'].lower() + '_' + otherPol['latest_member']['name']['last'].lower()
				data['party'] = otherPol['latest_member']['party']
				data['state'] = otherPol['latest_member']['electorate']
				data['cleanName'] = otherCleanName
				data['memberId'] = otherPol['id']
				data['name'] = otherPol['latest_member']['name']['first'] + ' ' + otherPol['latest_member']['name']['last']
				data['agreementCount'] = 0
				data['presentCount'] = 0
				voteChecks[otherCleanName] = data
		...
		for id in divisionIds:
			...
			with open(fileLoc,'r') as jsonFile:
				...
				#Check if senator present at vote
				...
				if polId in senatorsPresent:
					#Senator was present, so calculate agreement
					print 'present'

					#Get senators positions

					for vote in votes['votes']:
						# print vote
						if polId == vote['member']['person']['id']:
							senatorPosition = vote['vote']
							
					print "position was", senatorPosition
					
					#Now check position against others

					for vote in votes['votes']:
						#Exclude the initial senator we're checking against, and senators that are no longer in the Senate
						if polId != vote['member']['person']['id'] and vote['member']['person']['id'] in currentSenators:

							checkSenatorName = vote['member']['first_name'].lower() + "_" + vote['member']['last_name'].lower() 
							# print checkSenatorName

							if senatorPosition == vote['vote']:
								#They agree, so increment the agreement counter and present counter
								print polName, "agrees with", checkSenatorName
								voteChecks[checkSenatorName]['agreementCount'] = voteChecks[checkSenatorName]['agreementCount'] + 1
								voteChecks[checkSenatorName]['presentCount'] = voteChecks[checkSenatorName]['presentCount'] + 1

							elif senatorPosition != vote['vote']:
								#They don't agree, so only increment the present counter
								print polName, "doesn't agree with", checkSenatorName
								voteChecks[checkSenatorName]['presentCount'] = voteChecks[checkSenatorName]['presentCount'] + 1
		...
		for key in voteChecks:
			print key
			agree = int(voteChecks[key]['agreementCount'])*1.0
			present = int(voteChecks[key]['presentCount'])*1.0
			print agree, present
			
			if present != 0:
				print agree/present*100
				voteChecks[key]['agreement'] = float("{0:.2f}".format((agree/present*100)))
			else:
				print None
				voteChecks[key]['agreement'] = None

			voteDataTemp = {}
			voteDataTemp['party'] = voteChecks[key]['party']
			voteDataTemp['name'] = voteChecks[key]['name']
			voteDataTemp['cleanName'] = voteChecks[key]['cleanName']
			voteDataTemp['state'] = voteChecks[key]['state']
			voteDataTemp['agreement'] = voteChecks[key]['agreement']

			voteData.append(voteDataTemp)

		cumulativeAgreement['voteData'] = voteData	
		
		totalAgreement[cumulativeAgreement['cleanKeyName']] = cumulativeAgreement

		print voteChecks		

newJson = json.dumps(totalAgreement, indent=4)
with open('crossbench-voting-new.json','w') as fileOut:
